...
- `get_mean_se_multi(formula, design, group_name, period)` — runs `svymean()` on one or more variables and returns a tidy data frame of means and SEs tagged with a group and period label.
- `get_comp_row(design, group_label, period_label)` — sex + race/ethnicity composition for one design ("nh" = non-Hispanic).
- `get_df_value(design, group_name, period, wage)` — mean childcare hours plus the implied daily and annual dollar valuation at a given hourly wage.
- `get_sex_comp(design, role_label)` — sex composition (share + SE) for one design, labeled by role.
